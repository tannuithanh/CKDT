...
if(isset($_POST['result'])) {
    ...
    $id="";
    include ('Library/Connect_DB.php');
    $sql = "SELECT managerfile.IDFile,managerfile.Deny,member.IDMember,member.FullName,dept.IDDept,dept.NameDept,position.NamePosition,DATE_FORMAT(gxp.TimeStampStart,'%H:%i:%s %d/%m/%Y') as TimeStampStart,DATE_FORMAT(gxp.TimeStampEnd,'%H:%i:%s %d/%m/%Y') as TimeStampEnd,gxp.Reason,gxp.NameMember,gxp.Note FROM managerfile,member,dept,managermember,position,gxp WHERE member.IDMember = managerfile.IDMember AND member.IDMember = managermember.IDMember AND dept.IDDept = managermember.IDDept AND position.IDPosition = managermember.IDPosition AND managerfile.IDFile = gxp.IDFile AND managerfile.IDFile='".$value."'";
    $query = mysqli_query($con,$sql);
    if($query->num_rows>0){
        while($row=mysqli_fetch_array($query)){
            $v = array("IDFile"=>$row['IDFile'],"Deny"=>$row['Deny'],"FullName"=>$row['FullName'],"NameDept"=>$row['NameDept'],"NamePosition"=>$row['NamePosition'],"TimeStampStart"=>$row['TimeStampStart'],"TimeStampEnd"=>$row['TimeStampEnd'],"Reason"=>$row['Reason'],"NameMember"=>$row['NameMember'],"Note"=>$row['Note'],"Check"=>callcheck($row['IDFile'],$row['IDMember'],$row['IDDept']),"Checked"=>check($row['IDFile'],callcheck($row['IDFile'],$row['IDMember'],$row['IDDept'])['IDMember']),"Approve"=>callapprove($row['IDFile'],$row['IDMember']),"Approved"=> check($row['IDFile'],callapprove($row['IDFile'],$row['IDMember'])['IDMember']));
        }
