...
if(isset($_POST['result'])) {
    ...
    $member = "";
    $flag=false;
    $sql = "SELECT managerfile.IDFuntion,funtion.NameFunction,funtion.NumberApprove,managerfile.IDMemberCreate,funtion.NumberApprove1 FROM managerfile,funtion WHERE managerfile.IDFuntion = funtion.IDFunction AND managerfile.IDFile='".$value['idfile']."'";
    $query = mysqli_query($con,$sql);
    if($query->num_rows>0){
        while($row=mysqli_fetch_array($query)){
            ...
            while($row2=mysqli_fetch_array($query2)){
                ...
                if ($iddept == 'D0002' || $iddept == 'D0010') { 
                if(Substr($value['idfile'],0,5)=='F0013'){
                    $sql5 = "SELECT * FROM approve WHERE IDFile ='".$value['idfile']."' AND No=3";
                    $query5 = mysqli_query($con,$sql5);
                    while($row5=mysqli_fetch_array($query5)){
                        $email3 = Getmail($row5['IDMember']);
                    }
                }
...
//Recipients
    ...
    if($mail!='') {
        ...
        $mail->addCC($email1);
        $mail->addCC($email2);
        $mail->addCC($email3);   // Add a recipient
        }
//Content
        $mail->isHTML(true);
